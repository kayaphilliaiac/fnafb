 ...
 * 
 * @param Pastebin
 * @desc Ativar o auto save/load das suas traduções?
 * @type Boolean
 * @on Ativar
 * @off Desativar
 * @default false
 * 
 * @param Developer API Key
 * @parent Pastebin
 * @desc Todo mundo que usa nossa API é obrigado a usar uma Chave da API do desenvolvedor válida. - Pastebin
 * @type string
 * 
 * @param Username
 * @parent Pastebin
 * @desc Nome de usuario
 * @type string
 * 
 * @param Password
 * @parent Pastebin
 * @desc Senha de usuario
 * @type string
 *
 * @param Compilador
 * 
...
(function () {
	...
	var params = (function () {
		...
		var file = PluginManager.parameters('DrXama_languageManager'),
			...
			pastebin = JSON.parse(file['Pastebin'] || false),
			pastebinDeveloperAPIKey = String(file['Developer API Key'] || ''),
			pastebinUsername = String(file['Username'] || ''),
			pastebinPassword = String(file['Password'] || ''),
		...
		return {
			...
			'pastebin': pastebin,
			'pastebinDeveloperAPIKey': pastebinDeveloperAPIKey,
			'pastebinUsername': pastebinUsername,
			'pastebinPassword': pastebinPassword,
	...
	// Pastebin API
	//
	// The static class that manages storage for raw files.
	...
	Pastebin.initialize = function () {
		Pastebin.createAPIUserKEY(() => {
			Pastebin.loadAllText(() => {
				Pastebin.saveAllTexts();
			});
		});
	};

	Pastebin.status = function () {
		return params.pastebin;
	}

	Pastebin.developerAPIKey = function () {
		return params.pastebinDeveloperAPIKey;
	}

	Pastebin.username = function () {
		return params.pastebinUsername;
	}

	Pastebin.password = function () {
		return params.pastebinPassword;
	}

	Pastebin.apiUserKey = function () {
		return Pastebin.data().api_user_key;
	}

	Pastebin.data = function () {
		let fs = require('fs'),
			file = 'save/pastebin.rpgsave';
		if (!this._data) {
			this._data = {}
			if (fs.existsSync(localPath(file))) {
				this._data = JSON.parse(LZString.decompressFromBase64(fs.readFileSync(file, 'utf8')));
			}
		}
		return this._data;
	}

	Pastebin.save = function () {
		let fs = require('fs'),
			file = 'save/pastebin.rpgsave';
		fs.writeFileSync(file, LZString.compressToBase64(JSON.stringify(Pastebin.data())));
	}

	Pastebin.createAPIUserKEY = function (callback) {
		if (!Pastebin.data().api_user_key) {
			var data = `api_dev_key=${Pastebin.developerAPIKey()}&api_user_name=${Pastebin.username()}&api_user_password=${Pastebin.password()}`;
			...
			xhr.addEventListener("readystatechange", function () {
				if (this.readyState === this.DONE) {
					Pastebin.data().api_user_key = this.responseText;
					Pastebin.save();
					callback();
				}
			});

			xhr.open("POST", "https://pastebin.com/api/api_login.php");
	...
	Pastebin.saveAllTexts = function () {
		var data = `api_option=paste&api_user_key=''&api_paste_expire_date=10M&api_paste_format=json&api_paste_private=1\
		&api_paste_name=DrXama_LanguageManager_${$gameSystem.getterLanguageSystem()}_${index}_${customtag}&api_dev_key=${Pastebin.developerAPIKey()}\
		&api_paste_code=${text}`;
	...
	Pastebin.loadAllText = function () {
		var data = `api_option=list&api_user_key=${Pastebin.apiUserKey()}&api_dev_key=${Pastebin.developerAPIKey()}&api_results_limit=1000`;
