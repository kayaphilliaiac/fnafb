...
Window_Status.prototype.drawParameters = function(x, y) {
    ...
    for (var i = 0; i < CCParams.length; i++) {
      var paramId = i + 2;
      var y2 = (y + lineHeight * i) + (lineHeight / 2);
      this.changeTextColor(this.systemColor());
      this.drawText(TextManager.param(CCParams[i]), x - 32, y2, 240);
      this.resetTextColor();
      var actorParam = Yanfly.Util.toGroup(this._actor.param(CCParams[i]));
      var dw = 2 * this.textWidth(Yanfly.Util.toGroup(this._actor.paramMax(i + 2)));
      this.drawText(actorParam, x + 240 - 32, y2, dw, 'right');
    }
};
