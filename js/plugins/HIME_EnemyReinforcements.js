...
Nov 21, 2015 - member ID's weren't normalized to 1-based
Nov 17, 2015 - fixed "needs alive" condition for checking if added
Nov 16, 2015 - initial release
...
(function ($) {
  ...
  Game_Troop.prototype.addEnemyReinforcement = function(troopId, memberId) {
    ...
    console.log($dataTroops[troopId].members);
    console.log(member);
  ...
  Game_Troop.prototype.isEnemyReinforcementAdded = function(troopId, memberId, needsAlive) {
    var enemies = this._enemies;
    for (var i = 0; i < enemies.length; i++) {
      if (enemies[i].troopId() === troopId && enemies[i].memberId() === memberId) {        
        /* Needs to be alive */
        if (needsAlive) {
