...
Window_PartyDetail.prototype.drawActorBasicInfo = function() {
    ...
    this.drawActorSimpleStatus(this._actor, xpad + 5, y, width);
};
...
Window_PartyDetail.prototype.drawActorParams = function() {
    ...
    for (var i = 0; i < CCParams.length; ++i) {
      ...
      var paramId = i + 2;
      var text = TextManager.param(CCParams[i]);
      this.changeTextColor(this.systemColor());
      this.drawText(text, rect.x - 5, rect.y + (12 * i), rect.width);
      var paramValue = Yanfly.Util.toGroup(this._actor.param(CCParams[i]));
      this.changeTextColor(this.normalColor());
      this.drawText(paramValue, rect.x, rect.y + (12 * i), rect.width, 'right');
    }
};
...
Window_PartyDetail.prototype.drawActorParamsTitle = function() {
    ...
    if (this._linesAvailable >= 7) {
      ...
      this.changeTextColor(this.systemColor());
      this.drawText(text, -4, wy, ww, 'center');
...
Window_PartyDetail.prototype.drawActorEquipsTitle = function() {
    ...
    if (this._linesAvailable >= 7) {
      ...
      this.changeTextColor(this.systemColor());
      this.drawText(text, ww + 12, wy, ww, 'center');
    } else if (this._linesAvailable === 4) {
      var text = Yanfly.Param.PartyDetailEquip;
      this.changeTextColor(this.systemColor());
      this.drawText(text, ww + 12, wy, ww, 'center');
...
Window_PartyDetail.prototype.drawActorEquipsList = function(equips) {
    ...
    for (var i = 0; i < equips.length; ++i) {
      ...
      if (this._lastSlot) {
        ...
        this.drawItemName(equip, wx, wy + (12 * i), ww + 5);
      }
      wy += this.lineHeight();
    }
};
