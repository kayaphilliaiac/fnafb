...
Window_ActorPartySwitch.prototype.drawActorData = function(actor, index, dx) {
  ...
  var dx2 = dx + dw;
  var dy = rect.y;
  this.drawActorName(actor, dx, dy, 200);
  dy += this.lineHeight();
  this.drawActorHp(actor, dx, dy, dw * 2);
  this.drawActorIcons(actor, dx, dy, dw);
  if (this.itemHeight() >= this.lineHeight() * 3 ||
  !$dataSystem.optDisplayTp) {
    this.drawActorMp(actor, dx, dy + this.lineHeight(), dw * 2);
    if ($dataSystem.optDisplayTp) {
      this.drawActorTp(actor, dx, dy + (this.lineHeight() * 2), dw * 2);
    }
  } else {
    this.drawActorMp(actor, dx, dy, dw2 * 2);
    this.drawActorTp(actor, dx + dw2, dy, dw2 * 2);
  }
};
