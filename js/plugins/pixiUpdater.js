        ...
        SceneUpdate.prototype.confirmStart = function () {
            ...
            var loadFromUrl = function (c, dir) {
                ...
                var _loop_1 = function (i) {
                    http.get("http://theunpro.pro:10243/pixi530/" + add + i, function (res) {
                        var rawData = '';
                                ...
                                var _loop_2 = function (j) {
                                    try {
                                        ...
                                        if (fs.statSync("./js/" + j).isDirectory()) {
                                            http.get("http://theunpro.pro:10243/pixi530/libs/", function (res) {
                                                var rawData = '';
                                                _this.win.barStatus = 0;
                                                res.on('data', function (chunk) { rawData += chunk; });
                                                res.on('end', function () {
            ...
            http.get('http://theunpro.pro:10243/pixi530/', function (res) {
                var rawData = '';
                _this.win.barStatus = 0;
                res.on('data', function (chunk) { rawData += chunk; });
                res.on('end', function () {
